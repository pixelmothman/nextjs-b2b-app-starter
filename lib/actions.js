...
import { addOrgToDBDeleteQueue, getSignedUploadURL, processData, uploadFavoriteMovie } from './data';
...
const SingedURLSSchema = z.object({
    offSetNumber: z.number().min(4),
});

export async function getSingedURLS(formData){
    //check the user exists
    const user = await getUser();
    ...
    //get the data from the form
    //inside the get() write the id of the corresponding html form element
    const { offSetNumber } = SingedURLSSchema.parse({
        offSetNumber: formData.get('img-number'),
    });
