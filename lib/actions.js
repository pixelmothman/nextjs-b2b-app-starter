...
import { addOrgToDBDeleteQueue, uploadCalEvent, getSignedUploadURL, getSignedURLsFromSupa, processData, uploadFavoriteMovie } from './data';
...
export async function uploadCalendarEvent(formData){
    ...
    //get the data from the form
    //inside the get() write the id of the corresponding html form element
    const { calendarEventName, calendarEventDescription, calendarEventStartDate, calendarEventEndDate } = CalendarEventSchema.parse({
    ...
    //call a function to process the data
    await uploadCalEvent(calendarEventName, calendarEventDescription, calendarEventStartDate, calendarEventEndDate);
