...
export async function fetchFavoriteMovies(){
    ...
    try {
        ...
        const { data: data1, error } = await supabase.from("fav_movies_table").select('movie_name').eq("org_id", user.userId);
...
export async function uploadCalEvent(calEventName, calEventDescription, calEventStartDate, calEventEndDate){
    //prevents the response from being cached
    noStore();
    try {
