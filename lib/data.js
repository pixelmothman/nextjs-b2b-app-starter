...
export async function fetchFavoriteMovies(){
    ...
    try {
        ...
        const { data: data1, error } = await supabase.from("fav_movies_table").select('movie_id, movie_name').eq("user_id", user.userId);

        //create an array with the favorite movies
        let movieArray = [];
        for (let i = 0; i < data1.length; i++) {
            movieArray.push({
                movieId: data1[i].movie_id,
                movieName: data1[i].movie_name
            });
        }
...
export async function deleteFavMovie(movieID){
    ...
    try {
        ...
        const { deleteFavMovie: error } = await supabase.from("fav_movies_table").delete().eq("movie_id", movieID).eq("user_id", user.userId);
